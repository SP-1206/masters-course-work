...
void add_to_table(symbol **table,char variable[256], int val,char type[25],char scope_type[25], char *in_register)
{
  ...
  strcpy(new_sym->scope_type,scope_type);
  strcpy(new_sym->in_register,in_register);
...
void print_table(symbol **table)
{
  ...
  for(i = 0; i < 47; i++) {
    ...
    while(walker != NULL) {
      printf(" %s: %d %s %s %s\n", walker->variable, walker->val, walker->type,walker->scope_type,walker->in_register);
      walker = walker->next;
    }
  }
  return;
