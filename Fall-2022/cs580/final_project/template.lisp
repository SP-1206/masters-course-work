...
;;;;
;;;; Connect-Four Player
;;;; Due Midnight, the evening of SATURDAY, DECEMBER 3.
;;;;
;;;;
...
;;;; will change the text :TEMPLATEp with your :firstname-lastname package name
...
;;;; (play-human-computer-game #'template:make-computer-move nil)
;;;;
...
(defpackage :aravind-sp
  (:use "COMMON-LISP" "COMMON-LISP-USER")
  (:export "MAKE-COMPUTER-MOVE"))

(in-package :aravind-sp)
...
(if (or (funcall terminal-p game) (>= current-depth max-depth))
 (setf fin (funcall evaluate game is-maxs-turn-p))
(if (funcall is-maxs-turn-p game)
(dolist (x (funcall expand game))
(let ((mm (alpha-beta x (+ 1 current-depth) max-depth is-maxs-turn-p expand terminal-p evaluate alpha beta)))
(setf alpha (max alpha mm))
(if (>= alpha beta) (setf fin beta) (setf fin alpha)))
)
(dolist (z (funcall expand game))
(let ((mm (alpha-beta z (+ 1 current-depth) max-depth is-maxs-turn-p expand terminal-p evaluate alpha beta)))
(setf beta (min beta mm))
(if (>= alpha beta) (setf fin alpha) (setf fin beta)))
)
)
)
fin
...
(defun vercon (game row column streak)
(let ((cc 0))
(do ((c row (+ 1 c))) ((>= c (height game)))
...
(defun horcon (game row column streak)
(let ((cc 0))
(do ((c column (+ 1 c))) ((>= c (width game)))
...
(defun checkcon (game streak value)
(let ((count 0) )
(dotimes (x (height game))
(dotimes (y (width game))
(if (= (aref (car game) y x) value)
(progn (setf count (+ count (vercon game x y streak)) ) (setf count (+ count (horcon game x y streak)) ) (setf count (+ count (diacon game x y streak))) )
)
)
)
count
...
(let ((m4 (checkcon game 4 1)) (m3 (checkcon game 3 1)) (m2 (checkcon game 2 1)) (o4 (checkcon game 4 -1)) (o3 (checkcon game 3 -1)) (o2 (checkcon game 2 -1)))
(- (+ (* m4 20) (* m3 15) (* m2 10)) (+ (* o4 20) (* o3 15) (* o2 10)))
)
