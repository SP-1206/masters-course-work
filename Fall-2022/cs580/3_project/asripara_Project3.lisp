...
;;;
;;; DUE: MIDNIGHT THE EVENING OF FRIDAY, NOVEMBER 18
;;;
...
(defvar best_index 0)
(setf best_index (random (length population)))
(defvar next_index 0)
(dotimes (x *tournament-size*)  
(let ((next_index (random (length population))))
  (if (> (nth next_index fitnesses) (nth best_index fitnesses)) (setf best_index next_index) )
))

(nth best_index population)
...
;;; benefit here
(defparameter return_list '())
(dotimes (z num) (push (tournament-select-one population fitnesses) return_list) )
return_list
)
...
(let (return_vec '())
(dotimes (z *float-vector-length*) (push (if (random?)(random *float-max*)(-(random *float-max*))) return_vec))
(coerce return_vec 'simple-vector)
)
...
(dotimes (x (length ind1)) (if (> *crossover-probability* (random 1.0)) (rotatef (elt ind1 x) (elt ind2 x))))  
...
;;; might be handy
;;; 3. See also SETF
(dotimes (x (length ind)) 
	(if (> *mutation-probability* (random 1.0))
	
	(let ((n (+ (gaussian-random 0 *mutation-variance*) (elt ind x))))
			(progn (while (not (and (>= n *float-min*) (<= n *float-max*)))
				 (setf n (+ (gaussian-random 0 *mutation-variance*) (elt ind x) )) 
)	(setf (elt ind x) n))
)
	)
)
...
(let ((child1 (copy-seq ind1)) (child2 (copy-seq ind2)) ) 
(progn 
...
(let ((population '()) (fitnesses '()) (fittest '()) (children '()))
(funcall setup)
(dotimes (q pop-size) (push (funcall creator) population ))
(dotimes (k generations) 
(setf fitnesses (mapcar evaluator population))
(setf fittest (funcall selector 20 population fitnesses))
(dotimes (x (/ pop-size 2)) 
(dolist (x (funcall modifier (nth (random (length fittest)) fittest) (nth (random (length fittest)) fittest))) (push x children))
)
(setf population children)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;REPORT;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Name: S.P. Aravind Sri Ram									   ;;
;; G#: G01301679									   	   ;;
;; This project unlike the second one (neural nets) was pretty easy to work with. I loved the fact ;;
;; I actually got to implement gaussian mutation and uniform crossover. Evolutionary stratergies   ;;
;; and genetic algorithms in general were a fascinating concept to study. The way they relate to   ;;
;; various concepts of biology and how we endup liberally borrowing some of their terminologies    ;;
;; is fun. Like the previous projects I split this one into multiple modules and built each part   ;;
;; recursively piece by piece. Lisp is a challenging language but the number of functions it has   ;;
;; and the number of ways it provides to solve a problem are amazing.				   ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;REPORT;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
